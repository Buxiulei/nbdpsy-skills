...
# 希望24（4001619995）判为停用，**推断级**：官网信息冻结于 2021、2023 年机构自述近半来电
# 无法接通——**查无官方停服公告**。⇒ 写稿用「多次无法接通、不再推荐」⛔ 不写「已停止服务」。
...
# 🩸 2026-09-02 加「不再推荐」（审稿判定：**改闸门 ⛔ 改措辞**）：今后写稿一律用
#    「多次无法接通、不再推荐，请改拨 12356」——旧措辞「已停止服务」是**推断级**结论
#    （官网停更 + 机构 2023 年自述接通率，**查无官方停服公告**），不该再当唯一写法。
# ⛔ **「无法接通」不进本表**：它描述的是**症状**，不是「这个号不再用了」。进表会放行
#    「4001619995 偶尔无法接通，多打几次」——那行仍在把死号递给读者。声明词**多一项 ＝
#    漏放行**（有洞、没有任何人会发现），⇒ 拿不准就不加。B 措辞里真正的声明词是「不再推荐」。
DEAD_HOTLINE_RETIRED_DECL = re.compile(
    r"已(?:停止服务|停用|停运|下线|不再服务)|停止服务|此前(?:写过|提到|使用)|更正|勘误|不再(?:可用|使用|提供|推荐)")
# 🩸 2026-09-02 补单独的「拨」——**收紧，⛔ 不是放宽**。加「不再推荐」之后，
#    「不再推荐，请拨 4001619995」变成「有声明词 + 动词表认不出『请拨』」⇒ **静默放行**，
#    自相矛盾句从此漏网。「拨」不在表里原本是**疏漏不是设计**：见下方 `is_retirement_citation`
#    与 tests 里那条反证的 8/29 注释——正因为它不在表里，审稿样本「不要再拨」才「碰巧放行」。
# ⚠️ 代价：劝阻式「⛔ 不要再拨 4001619995」随之变假红，与「不要再拨打／别再致电／不必再联系」
#    同一口径（审稿 1.5 已把劝阻式假红定为**已知代价**），这里只是把覆盖面补齐，⛔ 口径没变。
DEAD_HOTLINE_RECOMMEND = re.compile(r"请?(?:拨打|拨号|致电|打|拨)|联系|求助(?:热线)?(?:：|:)")

DEAD_HOTLINE_DETAIL = (
    "停用热线回流：希望24已于2026-08-14证据停用，用 010-82951332 替换。"
    "⚠️ 若本行是**更正声明**（告诉读者这条热线已停），两个条件都要满足才放行："
    "① 同一行写上「不再推荐／已停止服务／此前写过／更正」这类声明词（现行写法用**不再推荐**）；"
    ...
    for i, line in enumerate(text.splitlines(), 1):
        if has_dead_hotline(line) and not is_retirement_citation(line):
            reasons.append(f"第 {i} 行出现**已停用的希望24热线**（4001619995）：{line.strip()[:40]}\n"
                           ...
                           f"把「不再推荐／已停止服务／此前写过／更正」写进同一行，"
                           f"**且替代号码写在推荐动词之后、其后不再出现死号**，即可放行")
