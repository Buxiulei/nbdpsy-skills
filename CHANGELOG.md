...
- **创作端** 「标题」节新增**判据 0**（排在原四条硬性判据之前）：标题必须场景自足；**硬线＝不许出现没有指代对象的代词/抽象词**（这种感觉／那件事／几十场／这一步），必须换成具体名词或场景；附本产线真实正反例对照表；交付前自检三问（谁在什么情况下／什么感受／为什么值得点开，任一答不出即退回）。**封面主标题同样适用且更严**（信息流里图先被看到）。
- **审查端**  新增判据 4.5，按 **blocker** 判。
