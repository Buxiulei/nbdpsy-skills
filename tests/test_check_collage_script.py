...
def test_停留不够的处置是加长本句_不是缩短别的():
    """⚠️ 这条写进报错是有意的：**⛔ 别为了凑时长去砍别的句子**——
    那正是老板 G9 批的「为了缩减字数而刻意删减」。"""
    f = cc.check(["短句", "也短"], focus=0)["fails"][0]
    assert "⛔ 不是把别的句子说短" in f and "语速问题" in f


def test_句数上限有依据_不是拍的():
    lines = ["这是一句足够长的话可以撑过三秒半的"] * 12
    f = [x for x in cc.check(lines, None)["fails"] if "上限" in x][0]
    assert "反推出来的" in f and "÷" in f
