...
def test_sha匹配不被正文里的电话号误命中():
    ...
    assert "4001619995" in 闸.记账词集(正文, 严格=False), (
        "CHANGELOG 里已经没有那串热线号了 —— 这条判据失去了它要防的真实样本，"
        "⛔ 别让它恒真地绿着。换一个正文里真实存在的、会被十六进制正则命中的数字串。")

    假sha = "4001619" + "a" * 33
    assert 假sha[:7] in 正文, "样本前提没了：那 7 位数字本应作为电话号的一部分出现在正文里"

    # 🔴 这条守的是**闸自己会被正文污染**，⛔ 不是匹配逻辑坏了。
    # 我写 v2.43.1 那节时亲手踩过：为解释这个洞而在正文里写下裸的 7 位数字，
    # 它当场变成合法「记账词」。⇒ 已改为只认反引号/代码块里的词，散文裸串一律不算。
    # 但**反引号里写裸串仍会开口子**（实测确认），这条守的就是那个残留窗口。
    # 先报这条，⛔ 别让人拿着下面「实现退回子串法」的提示去翻根本没坏的匹配逻辑。
    assert 假sha[:7] not in 词, (
        f"CHANGELOG 里出现了写在反引号或代码块里的裸十六进制串 `{假sha[:7]}`，"
        "它已被当成合法「记账词」——任何以它开头的提交从此白嫖。"
        "⇒ 用文字描述这个前缀，⛔ 别把那串数字放进反引号。⛔ 不是匹配逻辑坏了，别去改 `已记账`。")

    assert not 闸.已记账(假sha, 词), (
        "sha 匹配被电话号误命中了 —— 说明实现退回成了「拿短 sha 全文搜子串」。"
        "必须是「提取独立十六进制词 + 要求全 sha 以该词为前缀」，"
        "否则任何以那串号码前 7 位开头的提交都能白嫖记账，且没有人会发现。")
...
def test_散文里的裸串不算记账_反引号与代码块才算():
    """判据 6：**闸门扫描的文件里写关于闸门的说明，说明本身会被扫描**。

    只加一条「守某个具体串」的断言不通用——电话号、日期、**别的仓库的 sha**
    （本文件真有一个 `a857341`，那是 dreamina CLI 的提交）都可能撞上。
    ⇒ 记账词只认写在反引号里或围栏代码块里的，散文裸串一律不算。

    ⚠️ 围栏代码块必须算合法记账位：把 `git log --oneline` 输出粘进代码块是合理写法，
    ⛔ 不能因为它没有单反引号就判漏记。
    """
    裸 = "本次顺带记 07593bc 这个提交。"
    引 = "本次顺带记 `07593bc` 这个提交。"
    块 = "```\n07593bc docs: 某某改动\n```"

    assert "07593bc" not in 闸.记账词集(裸, 严格=True), (
        "散文里的裸十六进制串被当成了记账 —— 通用防护失效，"
        "电话号/日期/别的仓库的 sha 都能给闸开口子。")
    assert "07593bc" in 闸.记账词集(引, 严格=True), "反引号里的 sha 必须算记账，否则正常写法全被误拦"
    assert "07593bc" in 闸.记账词集(块, 严格=True), (
        "围栏代码块里的 sha 必须算记账 —— 粘 `git log` 输出是合理的记账写法，"
        "判它漏记就是假红。")
    assert "07593bc" in 闸.记账词集(裸, 严格=False), (
        "宽松词集必须仍能认出散文里的串 —— 它是报错时区分"
        "「压根没写」与「写了但写在散文里」的唯一依据，⛔ 别让两种错报成同一句话。")
