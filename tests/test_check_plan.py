...
def test_粒度目标区间_太碎和太长都提示():
    ...
    （**正是老板批的"节奏非常快"**）⇒ 粒度须在两者之间。
    ⛔ 区间外只 warn（内容有长有短），低于 3.5s 才拒。

    🔴 **2026-08-19 区间从 4–6s 改成 3.5–10s（助理拍板）——原因是「前提被否了」，
    ⛔ 不是「嫌它报红」。**4–6s 建立在「可以在句内按语义切分」的假设上，
    而 `tpl-paragraph` 硬契约⑥「段落边界只落在句边界上」把这个假设否了
    ⇒ **编排单元最小就是「句」**，X1–X4 实测每句均 5.8–8.2s。
    ⚠️ **放宽的是 warn 的范围，⛔ 不是 fail 的门槛**——下限仍是 3.5s 且照旧 fail
    （本用例第一条断言就在守它）。"""
    cues = [{"text": "感觉好了不代表能停药", "start": 0, "end": 12.0}]
    ...
    r9 = mk(0, 9.0)
    assert r9["ok"] and not r9["warns"], "9s 是「句」的常态长度，⛔ 不该再 warn"
    r12 = mk(0, 12.0)
    assert r12["ok"] and "偏长" in r12["warns"][0], "超上限该 warn 但不拒"
