...
def test_否定词不许做上屏结尾_但可做下屏开头():
    """🩸 **X1 样片实证（2026-08-19）**：`好得不像真的` 被切成
    「所以如果你发现自己总在好得**不**」｜「像真的」——上屏以「不」收尾，
    读者那一屏读到的是「**好得不**」，⚠️ 而它的自然理解是"好得**不**够"，
    **与原意正好相反**。
    ...
    （与「持续的｜情绪困扰」同构）⇒ **词边界仍然只是必要条件**。

    ⚠️ 这条是**有根据的不对称**，⛔ 别按 STRUCT_PARTICLES 那样前后都禁：
    「不是这样」做下屏开头**完全成话**，禁了白丢一批合法断点。"""
    pytest.importorskip("jieba")
    r = bo.soft_split("所以如果你发现自己总在好得不像真的", 14)
    assert r is None or not any(x[-1] in bo.NEG_TAIL_BAN for x in r[:-1]), \
        f"否定词做了屏尾：{r}"
    # 反向：做开头仍放行
    r2 = bo.soft_split("这件事其实没有你想的那么复杂真的", 12)
    assert r2 and any(x.startswith(("没", "不")) for x in r2[1:]), \
        f"否定词做开头被误禁了：{r2}"


def test_否定词禁令在产稿零影响_但规则确实生效():
    """⚠️ 「零变化」必须连着「量具自证」一起报，⛔ 否则它看起来像强证据。

    **29 份在产稿实测：断不开 62→62、软断 147→147**（那批稿里没有否定词做屏尾的情况）；
    而单例验证证明规则生效（上面那条用例）。
    🩸 中间还量错过一次：**拿 15 份 cues.json 的数去比 29 份 txt 的基线**，
    得出"断不开 62→27"的荒谬结论——⚠️ **加严之后数字下降，本身就该立刻起疑**。"""
    assert bo.NEG_TAIL_BAN == ("不", "没", "别", "未", "非", "无")
    src = (SCRIPTS / "build_oneline.py").read_text(encoding="utf-8")
