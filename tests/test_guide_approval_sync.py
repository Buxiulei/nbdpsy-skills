...
EXEMPT_SCRIPTS = {
    "menu_ops.py":
        "**豁免的唯一理由是机械上的**：它没有 `actions` 分发表（`main()` 里是另一种写法），"
        "本判据「命令 → 处理函数」的映射方式对它不适用。"
        "🔴 **⛔ 不要再拿「可逆」当豁免理由——那条已于 2026-09-02 被证伪**："
        "① `--delete` 调 `/cgi-bin/menu/delete`，微信 API 语义是**默认菜单和全部个性化菜单一起删**"
        "（`menu_ops.py` 自己的警示行已写明）；② `--get` 存的基线**不含个性化菜单**"
        "（`fetch_menu` 只取 `menu.button`，`conditionalmenu` 只用来打一句提示就丢掉）；"
        "③ 全仓 `addconditional` **零调用** ⇒ 本 skill **结构上没有重建个性化菜单的能力**；"
        "④ 文档约定的基线文件 `{workspace}/wechat/menu.json` **当前根本不存在**，也从未入过任何提交。"
        "⇒ `--delete` 删掉的东西**有一部分本 skill 恢复不了**，与群发的不可逆**不再是**「不是一回事」。"
        "🔴 **管制状态（要不要纳入坐标制）仍是裁定问题⛔不是工程问题，2026-09-02 已上报、尚未裁定**，"
        "未裁前⛔不自行扩范围——本条只订正说法，⛔ 没有改变它的管制状态。"
        "⚠️ 豁免的含义是「本判据不校验它」，⛔ 不是「它没有危险动作」"
        "（照上面④条，它恰恰有一个恢复不了的危险动作）。",
}
