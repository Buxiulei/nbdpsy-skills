...
> 审查对象：seo-artical-creator 产出的 pillar 长文 Markdown（含 frontmatter）。
> 立场：预设有问题，逐条找证据；不确定按 FAIL。

## 确定性检查（先跑，JSON 输出与 exit code 原样记入报告）

路径说明：`../seo-artical-creator/` 指安装后与 content-reviewer 同级的兄弟 skill 目录；命令在本 skill 目录下执行，或换算为绝对路径。

| 命令 | 预期 | exit 语义 |
|------|------|-----------|
| `python3 ../seo-artical-creator/scripts/count_hanzi.py <文件>` | 纯汉字 3000–5000 | JSON 输出；区间外 exit 2 |
| `python3 ../seo-artical-creator/scripts/check_links.py <文件>` | dead 为空 | JSON 输出；有 dead 链接 exit 1（`suspect` 不算 dead，但须逐条人工点开定性后写入报告） |
...
3. **citations 与正文引用一一对应**：参考文献区每条至少被正文引用一次（无未引用文献），正文每处引用标记都能落到参考文献区（无孤儿引用）。列出不对应的条目。
