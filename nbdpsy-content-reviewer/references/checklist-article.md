...
10. **危机声明在文末**：必须含全国统一心理援助热线 **12356** 与北京心理危机研究与干预中心热线 **010-82951332** 两个号码（参考口径：`本文不构成医疗建议；如处于心理危机请拨打全国统一心理援助热线 12356 或北京心理危机研究与干预中心热线 010-82951332（24小时）；紧急情况拨打 110/120`）。缺任一号码或不在文末 = FAIL。
    - **危机声明号码核对：以本条为唯一真源**，发现 4001619995/希望24 或 12356 配「24小时」字样一律 FAIL（12356 官方口径为每日≥18小时，只有 010-82951332 官网明写 24 小时）。⚠️ **「希望24 已停用」是推断级结论**（依据＝官网信息停更 + 机构 2023 年自述近半来电无法接通，**查无官方停服公告**）⇒ **写作用词一律「多次无法接通、不再推荐」**（如「希望24热线 4001619995 多次无法接通、不再推荐，请改拨 12356」），⛔ 不写「已停止服务」——那是把推断当事实。
    ...
    **没有库访问权时（运营机独立安装）的等价做法**：
    ...
    - **回读先核对身份再采信 0 命中**：确认页面里的 `ld-post-{slug}` 或 canonical 与请求的 slug 一致——身份不核，「0 命中」可能只是扫错了页。
      **⚠️ 三条抓取纪律（2026-08-16 一次误报事故的产物；那次「串页」经双方物证定性为抓取侧假象，服务端 280 次请求+79 个 ISR 产物零错配）**：
      ① **一个响应一个唯一文件名**（带时间戳或随机后缀），⛔ 禁止跨命令复用固定名如 `p.html`——事故现场读到的是 **14 天前**同名残留文件；
      ② **抓取与身份核对必须在同一条命令里完成**，⛔ 别分两条：中间隔一个 `cd`（尤其 `cd A 2>/dev/null || cd B` 这种带回退的）就可能换了目录，读到别人的产物，而状态码却来自你这次的请求——**两次操作的输出被拼成一次响应**；
      ③ **写事故时间前用磁盘产物 mtime 交叉验一次时区**：agent transcript 时间戳是 **UTC**（`…T11:17Z` ＝ CST 19:17），换算反了会拿错时间去翻生产日志、翻到空档，然后把「查不到」当成「没发生」（本仓已栽四次）。
