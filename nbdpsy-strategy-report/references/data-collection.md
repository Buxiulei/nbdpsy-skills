...
## ⚠️ 小红书互动数据污染（2026-07-31 分界，永久有效）

2026-07-31 起服务端上线了矩阵自动互动（互赞收藏/历史补量；自动评论 08-04 已摘除），**创作中心读出的赞/藏/评含自家 7 个矩阵号的互动**。采集铁律：
- 对外/报告指标一律用 `note_metrics_daily` 在 **2026-07-31 及之前**的快照（机制上线前的天然分界线）；
- 之后的月份用派生口径：外部互动 = 当日快照增量 −（note_interactions.done + browser_jobs 里 matrix_interact/note_comment_task 的 done 留痕）；
- 报告引用任何「收藏型内容有效」类结论时，用夕夕号 2026-02~05 的纯自然存量数据做证据（662/451 收藏那批），不要用 7/31 后的当前值；
- 人工手点互动无留痕（补量报「已点赞/已收藏」即其证据），只能推算不能逐条核对，报告须声明该盲区。
