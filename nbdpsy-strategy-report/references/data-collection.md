...
- **「系统自动取消（支付超时）」是循环预约的正常催付机制**（来访未按时付款→按规则取消并退已付部分，配套已有催付短信），老板已拍板「这个操作是正常的」。分析时归类为**付款流失信号**（老客连续多次未按时付款=留存线索，转接待跟进），**绝不能写成"产品缺陷/流程漏损/工程要修"**——把正常机制误判为缺陷会误导工程排期，2026-08-05 已为此挨过一次纠正。
