...
2. `motif` —— 用哪个动效件。**闭集，只能从这些里选**：
   {"｜".join(cp.MOTIFS)}
3. `emphasis` —— 本段要强调的词。**必须是本段文字的连续子串**（逐字对得上），
   没有值得强调的词就**填空字符串**（留空是合法的，⛔ 别硬凑）。

   🔴 **强调词会被放大上屏、脱离句法被单独看见**，所以有两条硬规则
   （2026-08-20 审稿实测，三处被拦下）：

   **① 引述句里，⛔ 绝不强调被引述/被反驳的那部分。**
   稿子里常有「网上有个说法：X」「网上常把这归结为 X」这种句子——**X 正是我们要拆掉的**。
   放大 X ＝ 我们在替它做断言。
   🩸 实证：`网上有个说法：他不爱你，是你不会引导他爱你。` 强调词抽了「**他不爱你**」
   ⇒ **开场第一屏（最像截图的位置）在对读者断言"他不爱你"**，而本条立意恰恰是
   「他爱不爱你，你答不了」。
   ⇒ 要么抽**我们自己的立场词**（如「不会引导」「归结为」），要么**留空**。

   **② 否定句里，⛔ 别强调"不是 X"那半，要强调肯定的那半。**
   🩸 实证：`所以不是支点多就不会疼，是别让一处的塌方，变成全部的塌方。`
   强调「**不是支点多**」⇒ 单看＝"支点多没用"，**把本篇的建议本身否定掉了**。
   ⇒ 抓「**别让一处的塌方**」这种**肯定的主张**。
   ⚠️ 但**不是见否定就躲**：`是同一个机制的合理延伸，不是已经被证实的结论。`
   强调「不是已经被证实」是**科学严谨性的诚实表述**，放大它反而是对的。
   **判据是「单独看见时它替我们说了什么」，⛔ 不是「含不含否定词」。**
