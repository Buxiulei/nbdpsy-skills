...
HOLD_FOCUS = 4.5
SPEED = 4.23
"""**句内**口播语速（汉字/秒）——15 份在产稿、401 句、8177 汉字、1933.8s 实测（2026-08-19）。

🔴 **语速有两个口径，⛔ 别混用**：

| 口径 | 值 | 算法 | 用途 |
|---|---|---|---|
| **句内** | **4.23** | `Σ(句 end−start) ÷ Σ字数` | **「这一屏停多久」← 本模块用这个** |
| 整片 | 3.88 | `(末句 end − 首句 start) ÷ Σ字数` | 「成片多长」 |

两者之差就是**句间静音**（合计 8.2%）。

🩸 **原来写的 3.5 是双重错配**（2026-08-19 用真 cues 重算才发现）：
① **口径错**——那是**整片**口径，却拿来算「每屏停多久」；
② **样本是异常值**——它出自 `oneline-qiuqiu-jianyao` 单条，而那条的句间静音是 **16.3%**，
   15 条里最高（其余都在 6–8%）。
⇒ 3.5 比真值低 17% ⇒ **每屏停留被高估 21%**：实际只停 3.5s 的屏被算成 4.2s ⇒ **闸门漏报**。
⚠️ **一条样本 + 错口径，出来的数看着很像真值**（它确实是"实测"的），
   这种数最难被怀疑——⛔ 别再拿单条片子定全线的尺子。

按本值折算：`HOLD_MIN 3.5s` ≈ **15 字**；`4–6s` ≈ **17–25 字**；印章屏 `4.5s` ≈ **19 字**。"""
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--speed", type=float, default=SPEED,
                    help=f"**句内**语速（汉字/秒，默认 {SPEED}＝15 份在产稿实测）。"
                         f"⛔ 别填整片语速 3.88——那是「成片多长」的口径，"
                         f"拿来算每屏停留会高估 9%%")
