...
HEDGE_CLASSES = {
    # 🔴 词表来自**稿库实证统计**（2026-08-18 扫 seo-geo/content 下 535 个 txt/md），
    # ⛔ 不是凭感觉编的。括号内为「总频次/出现文档数」。
    "频率": ("往往",      # 422/145
             "常常",      # 286/123
             "通常",      # 250/93
             "多数",      # 163/88
    ...
    "证据": ("有限",      #  54/41
             "类比",      #  51/32
             "可核对", "可观察", "初步", "尚未证实", "相关不等于"),
    # ⛔ 已删（实证零命中＝编出来没人用过）：一般来说／不见得／一定程度上／某种程度上
    # ⛔ 不收单字词（能／可／只／若／更）：误报会多到没人肯看——这是秘书 2026-08-18 的提醒，
    #    我的解法是**不收它们**，⛔ 不是改成需要句对应的局部比对（那个做不到）。
}
"""⛔ **按类别比，不按词比**——「往往」改说「多半」是合法的同义替换，按词比会误报到没人肯看。
只有**整类对冲在下游全部消失**才报。

## 它治的病（2026-08-18 X4 批次独立审查，跨三稿同向漂移）

**压句长时，对冲词是最先被牺牲的那一类。**三条实证：

- 🩸 **X4（最狠的一条）**：正文「它有**可核对**的结果」→ 口播「后者**有结果**」。
  **删掉三个字，从「你能看清他做没做」变成「这么说他就会做」**——直接撞红线
  「⛔ 不承诺效果」。**而且它是全片最后一句、听众带走的那一句。**
- X1：正文「**往往**是那些没有恋爱意图的日常来往」→ 口播删「往往」，变成无条件因果断言。
- X3：条件句变断言（⚠️ 这一条**机器抓不到**——没有对冲词可抓时，强度差是语义的）。

## ⚠️ 还有一种更隐蔽的，同一个函数能抓

**对冲句根本没进发布层**：X4 报告 4.2——边界句只写在给制图/内审的「要点」里，
**发布正文里没有**。⇒ **内审看要点觉得"边界写了啊"，读者看正文却读到一个效果承诺，
双方都以为有人管了。**
所以本函数收的是**任意上下游两层文本**（要点→正文、正文→口播都能用），
⛔ 不写死成「正文 vs 口播」。
"""


def hedge_gap(source: str, downstream: str) -> list[dict]:
    """上游有、下游整类消失的对冲类别。⛔ warn 用，不拦。

    ⚠️ 会漏也会误报：漏——下游换了本表没有的说法；误报——上游那个对冲在下游语境里不适用。
    ⇒ 定位是**把可疑处摆到人眼前**，判还是人判（与十四律软提醒同一档）。
    """
    out = []
    for cls, words in HEDGE_CLASSES.items():
        up = [w for w in words if w in source]
        if up and not any(w in downstream for w in words):
            out.append({"class": cls, "in_source": up})
    return out
...
⚠️ **触发条件：只在「一次写多篇」时才指望得上它**——单篇作业里 intent 串不到别的稿子上。
⇒ **批量写多篇时，本闸门会顺带拦住 intent 写串篇**；单篇时它就只是结构骨架检查。
...
def report(res: dict) -> None:
    ...
    hg = res.get("hedge_gap")
    if hg:
        _err(f"\n⚠️ 对冲强度比对：上游有、下游**整类消失** {len(hg)} 类（**只报不拦**，人判）：")
        for g in hg:
            _err(f"  · 「{g['class']}」类全没了——上游用过：{'、'.join(g['in_source'])}")
        _err("  🩸 真实事故（2026-08-18 X4）：正文「它有**可核对**的结果」→ 口播「后者**有结果**」。")
        _err("     删掉三个字，从「你能看清他做没做」变成「这么说他就会做」——撞红线「⛔ 不承诺效果」，")
        _err("     而且它是全片最后一句、听众带走的那一句。")
        _err("  ⚠️ 也可能是合理的：下游换了本表没有的说法，或那个对冲在下游语境里不适用。**人判。**")
    elif hg == []:
        _err("\n✅ 对冲强度比对：没有整类消失。")
...
def main() -> None:
    ...
    p.add_argument("--source-text",
                   help="上游文本（正文/要点）路径。给了就做**对冲强度比对**："
                        "上游有、下游整类消失的对冲类别 → warn（⛔ 不拦）。"
                        "任意上下游两层都能用：要点→正文、正文→口播")
    ...
    if a.source_text:
        try:
            src = Path(a.source_text).read_text(encoding="utf-8")
        except Exception as e:
            _err(f"⛔ 读不到 --source-text：{e}")
            sys.exit(2)
        res["hedge_gap"] = hedge_gap(src, "".join(t for _, t in items))
