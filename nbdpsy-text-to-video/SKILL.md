...
**三个平台都没有内容发布 API**——视频号是 2026-08-07 实调定案（服务号 token 调视频号接口回
`errcode 48001 api unauthorized`，连读粉丝数都不给）。**发布只能人工**，⛔ 不碰浏览器自动化
刷发布（账号是核心资产）。本 skill 能做到的上限是**出成品包 + 把该拦的在上传前拦住**：
