...
- 出现人物时指定**东亚面孔、自然真实**；心理内容优先用**空镜/隐喻/背影/手部特写**，
  规避真人肖像与审核风险。
- 美学统一：温暖、柔光、低饱和、治愈感，贴合 NBDpsy 品牌（具体色板走运营风格档案）。
...
**视频号没有任何内容发布 API**——2026-08-07 实调定案：拿服务号 access_token 调视频号接口
回 `errcode 48001 api unauthorized`，连读粉丝数都不给（开放平台视频号权限集只有留资/直播数据/
达人/橱窗 4 个，视频号助手 API 约 20 个接口里没有发布/上传）。**发布只能人工**在
channels.weixin.qq.com 做。⛔ 不碰浏览器自动化刷发布——账号是核心资产。

所以本 skill 能做到的上限是**出成品包**，把该拦的在上传前拦住：

```bash
python3 {SKILL_DIR}/scripts/shipinhao_pack.py \
  --video <成片.mp4> --title "标题" --text 文案.txt --out {workspace}/videos/<slug>/视频号包/
python3 ... --fix          # 规格不合格时自动转码修复（h265/HDR → H.264+SDR）
python3 ... --cover-at 3.5 # 指定抽封面的秒数（默认 2.0s）
```

产出：`video.mp4` / `cover.jpg` / `标题.txt` / `文案.txt` / `上传清单.md`。**有硬问题 exit 1**。

> ⚠️ **发小红书时封面必须单独做成 3:4 竖版**（1080×1440），别拿视频首帧或 9:16 图当封面——
> 小红书信息流按 3:4 展示，9:16 会被上下裁、标题字被切掉（2026-08-08 首批 8 条实翻车）。
> 视频本身仍是 9:16，封面是独立的图。**已发布笔记的封面改不了**（server 的 note-components
> 无 cover 参数，只有发布时的 publish-jobs 能设），**发布前必须核对封面比例**。
> 封面排版沿用小红书封面三级层级规范（超粗强调句 / 议题词 / 细体副标题），
> 文字压在底部半透明亮条上、别盖住人物面部。

两类硬拦（依据是视频号官方格式说明与《视频号医疗健康行业公约》）：

- **编码/色彩**：官方明文「暂不支持上传 HDR 视频」；h265 在 Chrome 上传会失败（官方建议换
  iPhone/Mac Safari——但直接转 H.264 更省事，`--fix` 干这个）。**别为此去买 Mac**：
  同一官方页写着「编码格式：不限」，说明是浏览器解码问题不是平台拒收。
- **绝对化用语**：「包治/根治/彻底治愈/100%/保证有效」等 + 引导脱离平台（「加微信/私信我」）。
  心理科普踩这条最容易，且是**账号级风险**，命中即 exit 1、必须回源头改。

我们两种形态的成片天然合规（笔记微电影 9:16、长文播客 720×1280，宽高比 0.5625 落在官方
0.33~3.0 区间内），正常不需要 `--fix`；它是给外部素材与手机直拍准备的。

**上传前必须念给运营的两句**：① **文案发布后终生只能改一次、每次 ≤20 字**，粘贴前通读；
② 逐条打**原创声明**（影响推荐权重与被引用能力）。

> 账号现状：视频号「NBDpsy-严选咨询师」已认证并绑定同名服务号。实测公域推荐可观——
