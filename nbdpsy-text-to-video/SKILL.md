...
**四种内容形态**（2026-08-07 命名定案两种＋2026-08-11 新增字卡＋2026-08-16 新增轮播放映）：
- **「笔记微电影」**——小红书笔记短内容 → 电影化动画短片（1–3 分钟，几秒一个分镜，
  Seedance 画面 + 口播旁白 + 分页字幕 + 品牌收尾）。本文十步产线就是它；创作方法论见
  `references/cinematic-direction.md`（含**子风格三档：暖雾·默认／沉静／晨光**，2026-08-16 老板定案，开工先问档位），旁白逐字稿规范见 `references/narration-spec.md`。
...
- **「轮播放映」**（2026-08-16 产线建成，老板同日验收样式通过，**可接单**；⚠️ 口播稿必须过外置审校才准出正式片——机械扩写稿只配做工程测试）——把**已产出的小红书轮播图**
  当 PPT 逐张放映 + 口播串联 + BGM，转成 3:4 竖版视频（`--canvas 1080x1440`）。**首版无字幕**
  （字幕会挡住信息图本身的字）。它是唯一**不新增画面成本**的形态：画面白嫖存量图文，
  **边际成本 ≈¥0.54/条**（口播 TTS）+ 本地渲染 ¥0，出片墙钟 **4 分 10 秒**——
  首片实测（H1 过度换气九页 / 806 字 / 成片 181.3s，样片 `seo-geo/content/videos/slideshow-proto/`）。
  一条命令收口：`scripts/slideshow_video.py --images-dir <图> --script-file <分页口播稿> --bgm auto --out <片>`
  （先跑 `--dry-run` 校验分页，不花 TTS 的钱）。规格见 `references/slideshow-video-spec.md`。
  ⛔ **选型判据**：轮播必须 R4 达标（每页有论点行、图内自足）才准进——**图讲不清楚，口播救不回来**，
  放映形态里画面是主体、口播只是导游，糊涂图配好口播只会做出一条糊涂片。
...
**§八 结构骨架三段也落成了闸门，办法是让答案可比对**（`--intent-file`，2026-08-17）：
写完稿回头把三句**成稿里的逐字原句**粘进一个 json 传给它——
...
原句能被字符串比对，**它才获得「12 字」那种质地——那个数之所以人人都记得，
就因为它是个可执行的数**。副作用是好的：**粘原句会逼你写完后真的回去找那一句，找不到＝没写。**

⚠️ **它的射程：只验「有没有」，⛔ 验不了「好不好」**——三样俱全的稿子照样可能因为句子被切碎
而不合格。**⛔ 别因为它没治好"读起来不通顺"就去加第四问第五问，那就变填表了。**
不传 `--intent-file` 时输出里**没有 `intent` 键**、stderr 明写「未做结构自证」——
⛔ 别读成「自证通过」。
...
> 🔴 **人工发也是不可逆动作**（老板 2026-08-27 定案）：发出去改不了、删了换链接、数据清零、
> 已分享的链接变死链。⇒ **发前须拿到老板台批复坐标**（件号 + `option_key`，形如 `<件号>-A`）。
> ⚠️ 这里没有脚本闸门可拦——**正因为是人工发，唯一的闸门是你这一问**。
> 把坐标记进投放留痕，⛔ 别口头「说过了」就发（口头令 ≠ 批复，要先补录上台、她/他点过才算）。
...
##### 🔴 补完封面**没有完**：素脸帖已被系统转私密，还要**人工转回公开**（server 0.24.18 起）

素脸哨兵确认真素脸的笔记，系统会**自动转成仅自己可见**（`note-visibility-changes` 台账里
`operator_id=0` 的系统行，⛔ 别当人工操作）。server 原话：
「修好封面后**需人工转回公开**——**别以为封面补上就全复原了**」。
⇒ ⛔ 只跑 `--fix-cover` 就报「处置完了」＝ 一条**发出去但没人看得见**的笔记。

```bash
# 完整处置 = 两步，🔴 顺序是**唯一安全序**，⛔ 不是推荐
python3 $XHS/publish_video.py --fix-cover --job <发布job_id> --cover <封面.jpg>   # ① 先补封面
python3 $XHS/note_ops.py --set-visibility --account <号> --privacy 0 --note-id <ID> # ② 再转公开
```

- 🔴 **为什么顺序是唯一安全序**（server 逐条实查，`queue_sentinel.py` 带行号）：
  ① 先补封面 ⇒ 凭据（note-components 带 cover 且 `applied.cover is True` 的任务行）落库**当轮即认**
  ⇒ 转公开时**零秒素脸公开面**。
  ⚠️ **反序（先转公开）不是「慢一点」，是「裸奔且无人再喊」**：那篇的素脸告警已 dedupe
  冻结出过声、**不会再响**；F 门（自动转私密）因幂等**永久关闭**（见下）⇒
  **反序产生的状态＝零保护＋零告警**。⚠️ 此时唯一的守门人就是本条顺序纪律——**没有机器兜底**。
- 🔴 **转回公开＝那扇保护门对这篇永久关闭**：F 的幂等判据是「该 note_id 存在过
  `target_privacy=1` 的台账行」（L720，转回公开不清除该行），⇒ 同一篇**终生只自动转私密一次**。
  **一个只保护一次的保护，和一直保护，是两件事**——转回公开后素脸风险回到人肩上，⛔ 别以为哨兵还在替你盯。
- ⚠️ **转完必须现读核一次**（note_ops 硬约束）：**`permission_code` 的 `null` 不是公开**
  （只有 `== 0` 才是，`1` 是仅自己可见）；**`success` 不等于生效**——这条产品线的失败普遍是静默的。
- ⛔ **批量转回公开只认 `operator_id=0` 的系统行降级过的那些**。server 原话逐字：
  「**『当前私密+现在有封面』的推断路线正是那次差点出事的形状，别走**」——
  运营**刻意**设为私密的笔记（草稿态/临时下架）⛔ 不许被顺手转公开
  （哨兵侧 `already_private` 门会跳过它们不入队，你这侧同样别碰）。
