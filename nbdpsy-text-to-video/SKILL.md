...
- **走图生 + 宿主没有图像生成能力（如 Claude Code）：⛔ 停等闸门（硬性协议，违反=事故）**——把 storyboard 绝对路径发给运营，并把**每一镜的生图提示词逐镜直接贴在会话里**，告诉运营（`<workdir>` 等占位符必须替换成真实绝对路径，别让运营看到尖括号/花括号）：「打开确认页逐镜核对脚本；复制每镜提示词生成 9:16 竖版图，命名成 P01.png 两位数（写成 P1.png 也认），放进 `<workdir>/images/`，全部放好回复我『图片好了』」。**说完立即结束你的当前回合**——不要继续第 3 步、不要假设图片已就绪。这是预期内的正常停等，不是任务失败。运营回复后先收图写回再核验：
...
- **走图生 + 宿主有图像生成能力（如 Codex）**：按每镜 `image_prompt` 逐镜生成 9:16 竖版图到 `<workdir>/images/P{页号}.png`，跑上面的 `--attach-images` 写回，同样把刷新后的 storyboard 路径给运营备查，然后继续。
...
- **image2video 画幅**：9:16 图 → 输出 720×1280（720p 档），画幅由图推断，prompt 只写运镜+微动作（无对白、画面无文字）。
