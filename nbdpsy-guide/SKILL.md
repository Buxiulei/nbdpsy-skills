...
| **把长文整篇排版成竖版文字长图**（不拆、不过 AI 生图，正文本身就是图） | 「**把这篇做成排版长图**」 | nbdpsy-xiaohongshu-creator（路线②）｜长文太长可再拆「上/中/下」系列篇 |
...
| 「**做成排版长图 / 文字长图 / 整篇发别拆 / 正文排版发出去 / 照那个文字版做**」 | 触发 nbdpsy-xiaohongshu-creator 走**路线②**（文本排版长图）：整篇长文精细排版成竖版长图，**不过 AI 生图**、图里就是正文本身。完整口径在那个 skill 的 `references/longform-typeset-spec.md` |
...
| 「拆小红书还是做长图 / 这篇适合哪种」 | 让 AI 按那个 skill 第 0.6 步的判据说人话给建议：**有完整论证链或必须整体呈现的表格 → 长图；能切出 5–8 个互不重叠的痛点场景 → 拆笔记**。**产出形态差很多（1 条 vs 5–8 条），最后让运营拍板** |
