...
| `T` | **实时**——截止目前 |
| `T-1` | **每天更新**——截至昨日 |
| `unknown` | **我们没核实过**。⚠️ **这不是默认值，别当成 `T` 或 `T-1` 去推算**；用到该字段时在结论里注明口径未知 |

### ⛔ 两条硬提醒

**① 跨 `window` 的字段相除，得不到真实比率**——分子分母不在同一时间窗。两个已知实例：

- **`views / exposure` 算不出 `cover_ctr`**（`views` 是 T、`exposure` 与 `cover_ctr` 是 T-1）
- **`follow_rate` 系统性偏低**（同一成因：分子分母跨窗）

**② `window: "unknown"` 表示我们没核实，不是默认值。** 别拿它当 T 或 T-1 去推算。
