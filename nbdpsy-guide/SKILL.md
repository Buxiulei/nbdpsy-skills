---
name: nbdpsy-guide
description: NBDpsy 内容运营工具包的「上手向导 + 客服台」，新运营不知道能干啥、怎么开始时用它。当用户说「教我用 / 怎么用 / 这个能干啥 / 帮我上手 / 新手引导 / 有哪些功能 / help / 我该做什么 / 从哪开始」，或说「更新工具包 / 更新 nbdpsy-skills / 升级 skill / 装最新版」，或问小红书运营操作「怎么装插件 / 怎么登录小红书账号 / 我有哪些账号 / 账号还能用吗 / 怎么发小红书 / 发布到小红书 / 怎么看笔记数据 / 拉取分析笔记数据」，或说「我要更新我小红书的风格 / 改一下我的风格 / 换个调性 / 我的风格跟别人不一样 / 我的风格档案 / 回到上个版本的风格 / 风格改回去」时，用本 skill。它介绍五个内容创作技能能做什么、怎么串成产线，手把手带做第一个任务，逐条给出 nbdpsy 小红书运营 API 的操作命令（装插件 / 登录 / 看账号 / 验活 / 发布 / 拉数据分析），并管理每个运营自己的风格档案（读取 / 更新 / 版本回退）。是非专业兼职运营的第一站；具体创作/发布仍由对应 skill 执行。
---
...
| `exists: false` | 「**你还没有自己的风格档案，先沿用管理员的那套，可以吗？**」 | 他说「可以」→ 就按管理员那套开工；他说「我想要我自己的」→ 第 3.6 步①建他的第一版 |
| 命令 **exit 2**（没配 key / 连不上 / 超时） | 「**没连上风格档案服务，本次用 skill 内置的默认风格（管理员那套）继续；等下次连上再核对**」 | 照常开工，别卡住产线。⚠️ 这句**必须说出口**，不许闷声降级 |
| 命令 **其它非 0 退出**（`exit 1` = 服务端报错，如 500） | **同上，说同一句话** | 同第 ③ 层处置。⚠️ 500 恰恰是"服务挂了"最常见的形态，别因为它不是 exit 2 就没了话术、自己发挥 |

> `exists:false` 时返回的 `profile` 是**管理员那套默认值**，不是他的——所以第二句话不能说成
> 「这是你的档案」。没配 `NBDPSY_XHS_API_KEY` 的运营（接入包可选、装的时候能跳过）走第三行，照样能做内容。

> ⚠️ **沿用管理员那套的运营一定会问「我沿用的那套以后会不会变」——会。** 管理员默认档案是**实时读的**，
> 不是他第一次读到时的快照：**管理员一改，所有还没建自己档案的人下次出图就跟着变**。想稳住就建一份
> 自己的（第 3.6 步①）——**一旦建了个人档案，管理员再怎么改都影响不到他**。完整说明见第 3.6 步③。
...
⚠️ 这句只是**念法的样子**，里头的值取自**管理员那套**。念的时候一律照**他自己 `profile` 里的实际取值**说——
...
## 第 3.6 步 · 改我的风格 / 退回上个版本 / 管理员默认档案

运营说「我要更新我小红书的风格」「改一下我的风格」「换个调性」「回到上周那版」时走这里。
...
### ③ 管理员默认档案（管理员才用；但每个运营都该知道它会变）

**没建个人档案的运营，用的就是这一份**（第 1.5 步 `exists:false` 那行），它归**管理员**维护：

```bash
python3 SP --admin-default /tmp/admin-default.json --note "本次改动说明"
```

- 打的是 `PUT /api/style-profile/admin-default`，**只有管理员能用（`require_admin`），普通运营会 403**——
  运营问起就照说「这套是管理员维护的，你改不了；但你可以建一份自己的，建了就归你自己管」。
  ⚠️ 这个 403 脚本回 **exit 1** 并明说「只有管理员能用」——**别把它当成"没连上"去念第 1.5 步那句降级话术**，
  服务好好的，只是这个人不是管理员。
- 同样是**整份覆盖**（先读全量 → 在全量上改 → 整份发回去）、version 自增，但**不带 `--base-version`**：
  它没有乐观锁，也就不会撞 409——正因为如此，两个管理员同时改会后写覆盖先写，**改之前先招呼一声**。

**传播语义三条（运营一定会问，别答错）：**

1. **没有个人档案的运营，每次 `--get` 都实时读到最新的管理员默认档案**，不是他建档/首次读到时的快照——
   **管理员一改，所有沿用者立刻跟着变**，下一批图就是新的样子。
   （`--get` 在 `exists:false` 时会带 `admin_default_version`，可以顺口告诉他「你现在沿用的是管理员那套的第 N 版」。）
2. **已经建了个人档案的运营各自独立、完全不受影响**——管理员怎么改都动不了他那份。所以运营担心
   「我的风格会不会哪天被人改掉」，答案是：**建了自己的就不会**。
3. ⚠️ **管理员默认档案不进版本历史表、不可回退**——`--versions` / `--rollback` 对它**无效**（那两个只管个人档案）。
   **改之前自己留底**：把这次要覆盖掉的那份原样存成一个文件收好，改坏了没有后悔药。
...
| 「我沿用管理员那套，以后会不会变 / 我的风格会被人改掉吗」 | 第 3.6 步③：**沿用的会跟着管理员实时变；建了自己的就完全不受影响**（想稳住就走第 3.6 步①建一版） |
