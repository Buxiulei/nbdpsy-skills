...
**口径的真源是 API 响应本身**——`--notes` 返回的 `meta.field_notes`（后续升级为逐字段的 `meta.field_meta`）里写着每个字段的定义、单位与**更新时间窗**。**分析前先读它**，别凭字段名望文生义，也别照搬本文档里的记忆值（平台口径会变，响应里的才是当下的）。

已核实的三个流量字段（2026-07-26 悬停数据看板 ⓘ 抄的官方原文）：

| 字段 | 平台官方说明 | 更新时间窗 |
|---|---|---|
| `exposure` 曝光 | 该篇笔记**截至昨日**获得的曝光数之和，每天更新 | **T-1** |
| `views` 观看 | 笔记**截止目前**的观看数，实时更新 | **T（实时）** |
| `cover_ctr` 封面点击率 | **截至昨日**的封面点击次数 ÷ 封面曝光次数（**视频下滑不计入**），每天更新 | **T-1 ÷ T-1**（内部自洽） |

其余指标（`likes` `collects` `comments` `danmu` `shares` `reposts` `follows` `avg_view_duration`）的时间窗**尚未逐个核实**——响应里没写就是没核实，**别假设它是 T 还是 T-1**，用到时在结论里注明这一点。
