...
发小红书靠自建的 nbdpsy-api（`https://mcp.nbdpsy.com`）+ 一个 Chrome 登录插件。六件事：
...
```bash
python3 PUB --notes <账号名或id>      # 拉该号已发布笔记的清单与互动数据
```

拿到数据后，你（Claude）直接**读 JSON 帮运营分析**：哪几篇浏览/点赞/收藏高、选题与钩子的规律、
发布时段效果、给下一批选题的建议。
> ⚠️ 此接口 server 端**正在上线中**；若命令返回「该能力还在上线中」，告诉运营「数据分析功能马上就好，
> 管理员上线后我就能拉」，别当故障。
