...
python3 SP --get        # 记下响应里的 base_version —— 下一步原样传它
                        # ⚠️ 认 base_version 这个字段，别去找 version：
                        # 他还没有自己的档案时（exists:false）响应里根本没有 version，
                        # 脚本已替你算好 base_version=0（首次建档就该传 0）
```
...
```bash
python3 SP --put /tmp/profile-new.json --base-version <A 读到的 version> --note "运营原话摘要"
```
...
```bash
python3 SP --versions          # ① 列历史版本：把版本号、时间、备注念给运营，让他挑一个
