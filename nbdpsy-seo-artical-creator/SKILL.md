...
SEO 让网页被搜索引擎收录；GEO（Generative Engine Optimization）让内容被 ChatGPT / 豆包 / 元宝 / 文心等 AI 引擎**抓取后愿意引用**。Princeton 的 GEO 研究实证（arXiv:2311.09735 表 2，按 SERP 排名分组）：**低排名页面**加带出处统计／署名引语／引用来源，可见性最高 **+97.9%~+115.1%**；🔴 **但排名第 1 的页面三者全为负（−20.6%~−30.3%）** —— GEO 是「弱者武器」，⛔ 别对已排第一的页面用。
