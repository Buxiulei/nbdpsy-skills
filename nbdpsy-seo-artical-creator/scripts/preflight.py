...
# 证据（**论文原表**：`arxiv.org/html/2311.09735v3`，KDD 2024，10,000 查询多策略对照）：
#   🔴 **唯一能直接引用的相对百分比在表 2（按 SERP 排名分组）**：
#     Cite Sources      Rank-1 **−30.3%** ／ Rank-5 **+115.1%**
#     Quotation Add.    Rank-1 **−22.9%** ／ Rank-5 **+99.7%**
#     Statistics Add.   Rank-1 **−20.6%** ／ Rank-5 **+97.9%**
#     Keyword Stuffing  Rank-1 −6.0%     ／ Rank-5 +6.1%
#   🔴 **三个主策略在 Rank-1 全为负**——**对排名最好的页面，这三招有害**。
#      ⛔ 别写成「对第 1 位几乎无变化」：那是**方向错**，会让人主动损害最好的页面。
#      ⇒ GEO 是「弱者武器」：低排名页受益最大，高排名页反受损。
#   ⚠️ 表 1 / 表 5 给的是**绝对分值**（Position-Adjusted Word Count / Subjective Impression），
#      ⛔ **不是百分比**。🩸 **刻意不写具体分值**：那两张表两次独立抓取结果不一致
#         （小模型读它们不稳定），而这句警告不需要具体分值也成立；写上就是又引入
#         一组只被单次读取确认过的数字。⛔ 除非有人逐字看过 PDF（我们都没有）。
#      摘要另有「up to 40%」，那是**整体上限**⛔ 不是单项收益。
#
# 🩸 **这组数我连错两版**（2026-08-25，两版都由**服务号线核出**）：
#   v1「引用来源 +31%／统计 +31%／引述 +41%」——两项对调、一项凭空；
#      还"拦下"一个 28%，理由「全文查无」——**而我的"全文"只是一个文件**。
#   v2 按本仓 §2.1 改成「统计 +41%／引语 +28%」——**仍是错的**：那两个数论文里根本没有，
#      表 1/5 是**绝对分值**，我把分值当成了百分比。
#   ⇒ **数字进规格前必须回到论文原表**——⛔ 二手转写（本仓 §2.1、调研文档）**都错了**；
#     且**每个数必须标到表号**，⛔ 否则「谁更有效」这个结论本身就不成立
...
def run(md_text: str, online: bool = False, api_base: str = DEFAULT_API_BASE):
    ...
    if geo_bad:
        add("R2-geo", "geo-open-structure", "fail",
            ...
            + "。实测（Princeton GEO，arXiv:2311.09735 v3，表 2 按 SERP 排名分组）："
              "低排名页受益大（Rank-5：引用来源 +115.1%、引语 +99.7%、统计 +97.9%）；"
              "🔴 但 Rank-1 三者全为负（−30.3%／−22.9%／−20.6%）——排名最好的页面别加",
    ...
    add("R2-geo", "geo-verifiable-params", "manual",
        ...
        "带出处的统计数字），⛔ 不是形容词。"
        "⚠️ 反例：**关键词堆砌几乎无用**（表 2：Rank-1 −6.0%、Rank-5 仅 +6.1%，"
        "远低于其它策略）——⛔ 别靠塞关键词凑")
