...
# 证据（真源：本仓 `seo-geo/research/02-GEO-海外引擎.md` §2.1，带 arXiv + ACM DOI 双链接）：
#   **Princeton GEO 论文（KDD 2024，arXiv:2311.09735，v3 2024-06-28）**，10,000 查询多策略对照：
#   **加带署名来源的统计数据 +41% ／ 加署名专家引语 +28% ／ 关键词堆砌 −10%**；
#   ⚠️ **加引用权威外部来源带条件**：低排名页面最高 **+115%**、**第 1 位页面几乎无变化**
#      ——⛔ 别写成裸百分比，收益取决于页面当前排名。
# 🩸 **这几个数我写错过一次**（2026-08-25，服务号线核出）：曾写成 31/31/41，**两项对调、一项凭空**；
#    还"拦下"过一个 28%，理由是「全文仅此一处、证据表里查无」——**而 28% 正是专家引语那一项**。
#    🔴 根因：`2311.09735` 在本仓有**两个来源文件**，我只 grep 了其中一个，
#    且没回头核 `pillar-spec.md` 第 3 行**早就写着**的「统计 +41%、引语 +28%」。
#    ⇒ **写一个数进规格前，先全仓搜它的出处，并核本文件既有段落有没有说过同一件事。**
#
...
def run(md_text: str, online: bool = False, api_base: str = DEFAULT_API_BASE):
    ...
    if geo_bad:
        add("R2-geo", "geo-open-structure", "fail",
            ...
            + "。实测（Princeton GEO，arXiv:2311.09735 v3）：带署名来源的统计数据 +41%、"
              "署名专家引语 +28%；⚠️ 关键词堆砌 −10%",
    ...
    add("R2-geo", "geo-verifiable-params", "manual",
        ...
        "带出处的统计数字），⛔ 不是形容词。"
        "⚠️ 反例：**关键词堆砌是负收益 −10%**（同一实验）——⛔ 别靠塞关键词凑")
