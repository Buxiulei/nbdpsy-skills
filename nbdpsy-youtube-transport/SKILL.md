...
description: 把一条 YouTube 视频「搬运」成带中文字幕/配音、可直接发布的成片——服务端全自动完成下载→转写→qwen-mt 翻译→豆包配音→音画同步→烧中文字幕→出成片，并自动打上 NBDpsy 品牌 logo（右下角）与片头版权声明。当用户说「搬运这个 YouTube 视频 / 把这条油管视频翻译配音 / 搬运油管 / YouTube 视频转中文 / 把这个视频做成中文版 / 搬个视频」并给出 youtube.com 或 youtu.be 链接时，用本 skill。它经小红书运营工具的 video-transport REST API 建任务、轮询进度、取回成片与中英字幕/双语逐字稿的公网下载链接。仅处理 YouTube 链接的搬运；从零创作竖屏视频请用 nbdpsy-text-to-video。
---
...
搬运接口和小红书发布接口是**同一套运营接入 JWT 鉴权**，复用本机凭据 **`NBDPSY_XHS_API_KEY`**。
配过小红书自动发布就能直接搬运，不用再要新凭据。自检：

```bash
python3 COMMON doctor          # 看 xhs_ready=true 即凭据就绪（同一把 key 也给搬运用）
```
...
- 命令报 `Host not allowed` / 恒超时 → Claude 沙盒拦网：`python3 COMMON sandbox allow` 写入放行名单
  （已含 `xhs.nbdpsy.com`），**重启 Claude Code** 后重试；单次命令也可用 Bash 工具参数
  `dangerouslyDisableSandbox` 兜底。
- `401` → apikey 失效/已轮换，找管理员重发接入包。

基址默认 `https://xhs.nbdpsy.com`（可用凭据 `NBDPSY_VIDEO_API_BASE` 覆盖，一般无需动）。
...
默认命令会**建任务 + 轮询到终态**（搬运耗时按视频时长几分钟级，脚本每 ~15s 轮询一次）：
...
| `--wait-timeout 秒` | 轮询等待上限 | 1800 |
| `--download` | 终态 completed 后把产物下载到本地工作区 | 默认只回公网链接（视频较大不自动下载） |
| `--out-dir 目录` | `--download` 落盘目录 | `<工作区>/video-transport/job-N/` |

**长视频/怕断**：加 `--no-wait` 先拿 `job_id`，之后用 `python3 TV --job <id>` 复查，避免长时间占用会话。

### 第 3 步 · 取回产物

stdout 是**纯 JSON**。终态 `outcome=completed` 时 `products` 里是**已拼成公网绝对 URL** 的四个产物
（**免鉴权**可直接下载/播放）：
...
```bash
python3 TV --list              # 列出我建的所有搬运任务（只属于本账号）
python3 TV --job 42            # 查某任务的状态/产物
python3 TV --retry 42          # 重试失败任务（重跑并轮询到终态）
python3 TV --delete 42         # 删除任务
```
...
| 用途 | 路径 |
|------|------|
| 建任务 / 轮询 / 取产物 / 任务管理（--url / --job / --list / --retry / --delete / --download） | `scripts/transport_video.py` |
| 凭据工具 / 沙盒放行（sandbox allow，已含 xhs.nbdpsy.com） | `scripts/nbdpsy_common.py` |

## 红线速记

1. **只搬 YouTube**：非 youtube.com/youtu.be 链接一律拒绝，不硬提交。
2. **状态未确认绝不重建任务**：`outcome=unknown` / 轮询超时 → 一律 `--job <id>` 复查，**绝不**重新 `--url` 提交同一条（会重复搬运、白烧算力）。
3. **发布是对外动作**：搬运只出成片与链接；要把成片发到小红书等平台，仍需运营确认账号与内容后，走对应发布流程。
4. **产物链接免鉴权公开**：`products` 里的 URL 谁拿到都能访问，回给运营时按需注意分享范围。
