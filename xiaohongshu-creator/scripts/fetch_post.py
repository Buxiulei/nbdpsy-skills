...
def list_posts(n: int = 10, api_base: str = DEFAULT_API_BASE) -> list:
    ...
    """
    url = f"{api_base}/api/public/blog/posts?page=1&page_size={n}"
    resp = fetch_json(url)
...
def main():
    ...
    try:
        ...
        if args.slug:
            result = fetch_one(args.slug, out_dir=args.out, api_base=api_base)
            print(json.dumps(result, ensure_ascii=False))
            return 0
        elif args.list is not None:
            result = list_posts(args.list, api_base=api_base)
            print(json.dumps(result, ensure_ascii=False))
            return 0
