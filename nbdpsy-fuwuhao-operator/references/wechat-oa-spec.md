...
| 败相 | 什么情况 | 信封 / 退出码 | 处置 |
|---|---|---|---|
| **结果未确认** | 拿不准生效没有。线索：**请求已发出**后出岔——读响应超时、连接断、服务端 5xx、响应解析失败；外加唯一特例 `45028` | `outcome: unknown`，exit 0 | **先查台账核实，绝不自动重试** |
| **结果已确定失败** | 能确定没做成。线索：**请求未发出**（鉴权 401/403、参数校验不过、连接未建立、缺凭据），**或**微信明确回了拒绝码 | `outcome: failed`，exit 1 | 改配置/参数再来；配置错误**绝不静默通过** |
