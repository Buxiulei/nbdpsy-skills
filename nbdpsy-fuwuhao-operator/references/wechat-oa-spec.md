...
| **发布(freepublish) ≠ 群发(mass)** | 发布不推送粉丝、不占次数；群发**每自然月仅 4 次**。⚠️ **两者都不可逆**——差别在推不推送、占不占配额，⛔ 不在能不能反悔 |
...
服务端 5xx 另带 **`sent` 判别位**：`false` = 请求根本没转到微信（确定没生效，归 `failed`）；
`true` 或**字段缺失**（旧版服务端）= 已发出、结果不明，归 `unknown`。判据只认显式 `false`——
把缺失也当成「没发出去」会正好搞反最贵的那一侧（可能已群发却报确定失败，诱导再发一次）。

非幂等动作（发布 / 群发 / 删除）的败相按**这次到底成没成、能不能确定**分两种（脚本实现契约）：

| 败相 | 什么情况 | 信封 / 退出码 | 处置 |
|---|---|---|---|
| **结果未确认** | 拿不准生效没有。线索：**请求已发出**后出岔——读响应超时、连接断、服务端 5xx（未带 `sent:false`）、响应解析失败；外加唯一特例 `45028` | `outcome: unknown`，exit 0 | **先查台账核实，绝不自动重试** |
| **结果已确定失败** | 能确定没做成。线索：**请求未发出**（鉴权 401/403、参数校验不过、连接未建立、缺凭据、5xx 带 `"sent": false`），**或**微信明确回了拒绝码 | `outcome: failed`，exit 1 | 改配置/参数再来；配置错误**绝不静默通过** |

具体错误码归桶：

| 码 | 含义 | 信封 | 处置 |
|---|---|---|---|
| `40164` | 调用 IP 不在白名单 | `failed` | 找管理员核对生产服务器出口 IP |
| `45009` | 接口调用频控 | `failed` | 等一等再试，别连续重跑 |
| `48001` | 接口未授权 | `failed` | 核对服务号认证与权限状态 |
| `53501` | 发布过于频繁 | `failed` | 隔开时间再发 |
| `45028` | 群发保护，需管理员手机 30 分钟内确认 | **`unknown`** | **不是失败**，通知管理员确认 |

**确定性 errcode 拒绝一律归 `failed`**（exit 1，透传 `wechat_errcode`/`wechat_errmsg`/`hint`）：微信明确说了
没做成，结果确定，**不查台账**——`unknown` 的意义是防「可能已生效」的误重试，确定拒绝没有这个风险。

**唯一例外 `45028`** → `outcome: unknown`（exit 0），它是真正的结果未确认。`hint` **固定为这句**：
