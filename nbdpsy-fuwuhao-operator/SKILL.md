...
- 发布（freepublish）后进公众号主页【贴图】栏，**不推送粉丝、不占群发次数、无次数限制**——
  贴图天然适合比文章更高的更新频率。群发贴图也可以（一条占 1 次月配额），规矩同红线①。
...
- 尺寸提醒：小红书图是 2:3 竖版，贴图封面在部分列表位按 3:4 裁剪展示——直接平移可用，
  关键信息别顶着上下边缘排。
- 2026-08-05 生产实测通过：8 图笔记平移，upload-material → draft/add(newspic) → publish 全链路 OK
  （台账 9）。
