...
PROFILES = {
    ...
    "xhs": {"modules": ["yaml", "requests"], "modules_optional": ["PIL"], "credentials": [],
            "credentials_optional": ["NBDPSY_XHS_API_KEY"], "cli": []},
...
def _check_module(name: str, optional: bool = False) -> dict:
    pip_name = MODULE_PIP_NAME.get(name, name)
    present = _module_present(name)
    status = "ok" if present else ("warn" if optional else "missing")
    detail = "已安装" if present else ("未安装（可选，缺它则该场景走人工降级）" if optional else "未安装")
    return {
        "name": name,
        "status": status,
        "detail": detail,
...
def _check_credential(key: str, optional: bool = False) -> dict:
    present = bool(nbdpsy_common.get_secret(key))
    status = "ok" if present else ("warn" if optional else "missing")
    detail = "已配置" if present else ("未配置（可选，缺它则该能力走人工）" if optional else "未配置")
    return {
        "name": key,
        "status": status,
        "detail": detail,
        "fix": "" if present else
               ...
               "向管理员索要「凭据配置包/接入包」后 secret import 导入",
    }
...
def run(profile: str, install: bool) -> dict:
    ...
    for mod in spec.get("modules_optional", []):
        c = _check_module(mod, optional=True)
        if install and c["status"] == "warn":  # 可选模块缺失记 warn，--install 仍自动补装
            _pip_install(MODULE_PIP_NAME.get(mod, mod))
            c = _check_module(mod, optional=True)
        checks.append(c)
    ...
    for key in spec.get("credentials_optional", []):
        checks.append(_check_credential(key, optional=True))
