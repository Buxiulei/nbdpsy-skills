...
def _已推送(sha: str):
    """HEAD 在不在任何远端分支上。True/False/None（None＝拿不准，⛔ 别当成「没推送」）。

    🔴 这是「平时红是常态」那条边界的**前提**，2026-09-02 补。那条常态之所以成立，
    靠的是「这个悬着的提交**还会**被下一个提交或发版收走」——而一旦 `git push`，
    它就进了远端历史，那个「下一个提交」**可能永远不来**。
    🩸 实证：`6282662` 当 HEAD 时闸红，按「只有 HEAD 一个是常态」放过去**并且推送了**；
    它当时零记账，直到别人的提交把它挤出 HEAD 位才暴露成「真漏了」。
    ⇒ 常态说明只对**未推送**的 HEAD 打印；已推送就是真漏，⛔ 不能被安慰过去。
    """
    r = subprocess.run(["git", "-C", str(ROOT), "branch", "-r", "--contains", sha],
                       capture_output=True, text=True)
    if r.returncode != 0:
        return None                      # 没有远端 / 远端引用不全：拿不准
...
🔑 ⛔ 别改本脚本的判据来放行。拦的就是「v2.42.0 后 7 个提交零记账、feat 静默积压五天」
   这件事——判据一放宽，下次积压的还是同一批东西。""")
        # 🔴 红灯侧的适用边界，与绿灯那块对称。
        # 主干最后一个提交永远未记账 ⇒ 平时跑一次看到红是常态，⛔ 不是闸坏了。
        # 不说破的话，下一个人会去修一个没坏的东西。
        # ⚠️ 条件必须窄到「只有 HEAD 一个、且它不是发版提交」——放宽一点，
        #    真正的漏记就会被这句话安慰过去，那比不打印更糟。
        head = _git("rev-parse", "HEAD").strip()
        # ⚠️ 顺带排除「HEAD 是 release 提交却仍漏记」——那是夹带实质文件的发版提交，
        #    是真问题，⛔ 不能被这句常态说明安慰过去。
        if (len(漏记) == 1 and 漏记[0][0] == head
                and not 漏记[0][1].startswith(RELEASE_PREFIX)):
            # 🔴 常态说明**只给未推送的 HEAD**（2026-09-02 补的限定语）。
            # 失效方向刻意选「不安慰」：拿不准时宁可让人自己去看，
            # ⛔ 也不要把一个已经推走的漏记安慰过去——后者正是 6282662 那次。
            已推 = _已推送(head)
            if 已推 is True:
                print("""
🔴 注意：红的虽然只有 HEAD 一个，但**它已经推送到远端了** ⇒ **这不是常态，是真漏了**。
   「最后一个提交悬着是常态」成立的前提是**它还在本地**、还会被下一个提交或发版收走；
   一旦推送，它就进了远端历史，那个「下一个提交」**可能永远不来**。
   ⇒ **现在就把账落了**（把它的短 sha 带反引号写进 CHANGELOG），⛔ 别等发版。""")
            elif 已推 is False:
                print("""
⚠️ 不过：红的只有 HEAD 这一个提交、**且还没推送**，你也还没发版 ⇒ **这是常态，⛔ 不是闸坏了**。
   主干最后一个提交永远处于未记账状态（补记账的提交自己又没记账），
   它的账在**下一个发版节**里落。「闸绿」只在发版那一刻有意义。
   ⇒ 现在**不用管**；但 🔴 **`git push` 之前必须把账落了**——推出去之后它就不再是
   「悬着的最后一个」，而是一笔没人会来收的漏记。
   （若你想让它当场绿：用一个**纯记账提交**收尾并带 `[no-changelog]`——
     🔴 该豁免只给纯记账提交，⛔ 判据不是「只动了 CHANGELOG.md」，
     顶部铁律块是规格、改它算实质改动。详见 README 发版规范。）""")
            else:
                print("""
⚠️ 红的只有 HEAD 这一个提交，而你还没发版——**这可能是常态**（最后一个提交总是悬着）。
   ⚠️ 但本次**没能判断它推送了没有**（没有远端，或远端引用不全），而那正是分界线：
   **未推送**⇒ 常态，账在下一个发版节里落；**已推送**⇒ 真漏了，现在就得补。
   ⇒ 自己核一眼（`git branch -r --contains HEAD`），⛔ 别默认它是常态。""")
        return 1
